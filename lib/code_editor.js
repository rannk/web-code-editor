...
function ce_editor(){
     ...
     * 添加新的文件到目录，并打开。
     */
    this.addNewFileToFolder = function (folder_id, file_id, callback) {
        var folder_li_id;
    ...
    this.onContentLoaded = function (f) {
        content_loaded_callback[content_loaded_callback.length] = f;
    }
    ...
    this.trigger = function(c) {
        ...
        if(c == "content.loaded") {
            for(var i=0;i<content_loaded_callback.length;i++) {
                content_loaded_callback[i]();
            }
        }
    }
}
...
$(document).ready(function(){
    ...
    //菜单放大缩小控制
    $(function(){
        var target = $("#workspace")[0];
        var minSize = 300;
        var _startX = 0;

        $("#workspace_handle").offset({"left": $(target).width(),"top":window.innerHeight/2});
        $("#workspace_handle").on('mousedown', downHandler);
        ...
        function moveHandler(e) {
            ...
            _startX = startX;
            $("#workspace_handle").offset({"left": $(target).width(),"top":window.innerHeight/2});
            $("#workspace").attr("selected_width", w + 50);
            $("#file_selected").width(w+50);
    ...
    $(".menu_wrapper li").each(function (index) {
        ...
        obj.click(function(){
            ...
            var has_sub = obj.attr("has_sub");

            if(has_sub == "true") {
                $(".menu_wrapper .sub").children("ul").hide();
                obj.children(".sub").children("ul").show();
            }else {
    ...
    $("#minimize_icon").click(function () {
        $("#touch_menu").hide();
        $("#touch_menu_icon").show();
    });

    $("#touch_menu_icon").click(function () {
        $("#touch_menu").show();
        $("#touch_menu_icon").hide();
    });
});
...
function getFileContent(content_id, code_editor) {
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(ext == "png") {
                $("#" + element).remove();
                obj.append("<div class='show_jpg'><img src='"+data+"'></div>");
            }else {
                ...
                myCodeMirror.on("change", function (cm, obj) {
                    ...
                    var reg = /^\W$/;
                    if(reg.exec(obj.text.toString()) != null) {
                        if(typeof cm.initClassHint == "function")
                            cm.initClassHint();
                    }
                    ...
                    // 收集页面信息，保存class信息，方便后面进行补全
                    if(typeof cm.HintCol == "function")
                        cm.HintCol(cm);

                    $.editor.trigger("content.change");
                })

                // 关闭workspace目录, 主要让移动端生效
...
function showActiveContent() {
    ...
    if(content_id) {
        ...
        if($( content_id + " .CodeMirror").length == 0 && $( content_id + " .show_jpg").length == 0) {
        ...
        $("#filename_show").html($("body").attr("project_title") + ": " + f);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data, function(idx, item){
                ...
                if(item.hasfile == "1") {
                    content += "<span class='ide-icon ide-caret-right tag' />";
                }else {
                    left = "left20";
                }
...
function addClick(element) {
    $(element).each(function(index){
        ...
        $(this).click(function(e){
            ...
            var selected_width = $("#workspace").attr("selected_width");
            if(!selected_width) {
                selected_width = 350;
            }
            $("#file_selected").width(selected_width);
            setTimeout(function () {
                check_double = 0;
            }, 500);
            ...
            if(check_double > 1) {
                ...
                if(obj.attr("type") == "folder") {
                    ...
                    if(child_obj.is(":hidden")){
                        obj.children("div").children(".tag").removeClass("ide-caret-right");
                        obj.children("div").children(".tag").addClass("ide-caret-down");
                        child_obj.show();
                        ...
                        obj.children("div").children(".tag").removeClass("ide-caret-down");
                        obj.children("div").children(".tag").addClass("ide-caret-right");
                        child_obj.hide();
                    }
                }
