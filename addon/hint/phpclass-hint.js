...
(function(CodeMirror) {
    ...
    function getHints(cm, options) {
        ...
        if(/\$\w.*/.test(token.string)) {
            ...
            hint_arr = getClassHintValue(v2.string, cur.line, token.start - v2.string.length);
            c_shown = true;
            hint_b = token.start+2;
            if(hint_arr) {
                return {list: hint_arr,
                    from: Pos(cur.line, hint_b),
        ...
        if(c_shown) {
            hint_arr = filterArr(token.string, hint_arr);
            if(hint_arr) {
                return {list: hint_arr,
                    from: Pos(cur.line, hint_b),
                    to: Pos(cur.line, token.end)};
        ...
        if(c_shown == false && /^\w.*/.test(token.string)){
            ...
            hint_arr = filterArr(token.string, [phpKeywords, phpAtoms, phpBuiltin].join(" ").split(" "));
            return {list: hint_arr,
    ...
    function filterArr(string, arr) {
        ...
        if(typeof $.hint_class_arr == "object") {
            for(var key in $.hint_class_arr["global_fun"]) {
                if($.hint_class_arr["global_fun"][key].indexOf(string) == 0) {
                    new_arr[j++] = $.hint_class_arr["global_fun"][key];
                }
            }
        }
    ...
    function getClassHintValue(val, line, cn) {
        var v_check, v_name, v_obj;

        if(typeof val_arr == "undefined")
            return;
        ...
        for(var i=0;i<val_arr.length;i++) {
            ...
            if(v_check['line'] > line)
                continue;


            if(v_check['cn'] > cn && v_check['line'] == line)
                continue;
        ...
        if(v_name && v_obj) {
            ...
            return getHintFuns(v_obj);
        }
    }

    function getHintFuns(obj) {
        var r_arr = [];
        var count = 0;
        if(typeof class_arr[obj] == "object") {
            for(var i in class_arr[obj]) {
                if(i == "trait") {
                    var arr = getHintFuns(class_arr[obj][i]);
    ...
    CodeMirror.registerHelper("hint", "php", getHints);

    //用来标示显示class的补全还是全局方法的补全
    CodeMirror.defineExtension("initClassHint", function () {
        c_shown = false;
    });

    $.ajax({
        url: 'index.php/api/getHintContent',
        type: 'GET',
        dataType: 'json',
        error: function(e){
            console.log(e);
        ...
        success: function(data) {
            console.log("php hint contents loaded.");
            var data_arr = eval(data);
            if(typeof $.hint_class_arr == "object") {
                for(var key in data_arr) {
                    $.hint_class_arr[key] = data_arr[key];
                }
            }else {
                $.hint_class_arr = data_arr;
            }
        }
    });
})
